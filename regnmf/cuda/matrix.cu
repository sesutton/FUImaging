...
template<unsigned int blockSize>
__global__ void reduce2DStrided(float *g_idata, float *g_odata, int N,
		int stride);
template<unsigned int blockSize>
__global__ void reduce1DDiff(float *g_idata1, float *g_idata2, float *g_odata,
		int N);
template<unsigned int blockSize>
__global__ void reduce1DDiv(float *g_idata1, float *g_idata2, float *g_odata,
		int N);
template<unsigned int blockSize>
...
void create_matrix_on_device(matrix* A, int rows, int cols, float value) {
	...
	if (err != cudaSuccess) {
		fprintf(stderr,
				"create_matrix_on_device: cudaMalloc: ErrorMemoryAllocation\n");
		exit(1);
	}
...
void create_matrix_on_both(matrix* A, int rows, int cols, float value) {
	...
	if (err != cudaSuccess) {
		fprintf(stderr,
				"create_matrix_on_both: cudaMalloc: ErrorMemoryAllocation\n");
		exit(1);
	}
...
void copy_matrix_to_device(matrix* A) {
	...
	if (A->mat == NULL) {
		fprintf(stderr,
				"copy_matrix_to_device: matrix not allocated on host\n");
		exit(1);
	}
	if (A->mat_d == NULL) {
		err = cudaMalloc((void**) &(A->mat_d), sizeof(float) * N);
	...
	err = cudaMemcpy(A->mat_d, A->mat, sizeof(float) * N,
	...
	case cudaErrorInvalidDevicePointer:
		fprintf(stderr,
				"copy_matrix_to_device: cudaMemcpy: InvalidDevicePointer\n");
		exit(1);
		break;
	case cudaErrorInvalidMemcpyDirection:
		fprintf(stderr,
				"copy_matrix_to_device: cudaMemcpy: InvalidMemcpyDirection\n");
		exit(1);
		break;
	}
}
...
void copy_matrix_on_device(matrix A, matrix B) {
	...
	if (A.mat_d == NULL) {
		fprintf(stderr,
				"copy_matrix_on_device: source matrix not allocated on device\n");
		exit(1);
	}
	if (B.mat_d == NULL) {
		fprintf(stderr,
				"copy_matrix_on_device: dest. matrix not allocated on device\n");
		exit(1);
	}

	err = cudaMemcpy(B.mat_d, A.mat_d, sizeof(float) * N,
	...
	case cudaErrorInvalidDevicePointer:
		fprintf(stderr,
				"copy_matrix_on_device: cudaMemcpy: InvalidDevicePointer\n");
		exit(1);
		break;
	case cudaErrorInvalidMemcpyDirection:
		fprintf(stderr,
				"copy_matrix_on_device: cudaMemcpy: InvalidMemcpyDirection\n");
		exit(1);
		break;
	}
}
...
void copy_matrix_from_device(matrix* A) {
	...
	if (A->mat_d == NULL) {
		fprintf(stderr,
				"copy_matrix_from_device: matrix not allocated on device\n");
		exit(1);
	}
	if (A->mat == NULL)
	...
	cudaError_t err;
	err = cudaMemcpy(A->mat, A->mat_d, sizeof(float) * N,
	...
	case cudaErrorInvalidDevicePointer:
		fprintf(stderr,
				"copy_matrix_from_device: cudaMemcpy: InvalidDevicePointer\n");
		exit(1);
		break;
	case cudaErrorInvalidMemcpyDirection:
		fprintf(stderr,
				"copy_matrix_from_device: cudaMemcpy: InvalidMemcpyDirection\n");
		exit(1);
		break;
	}
}
...
void copy_to_padded(matrix A, matrix Apad) {
	...
	if (M > M_padded) {
		fprintf(stderr,
				"copy_to_padded: padded number of rows must be >= original\n");
		exit(1);
	}
	if (N > N_padded) {
		fprintf(stderr,
				"copy_to_padded: padded number of cols must be >= original\n");
		exit(1);
	}

	cudaError_t err;
	err = cudaMemcpy2D(Apad.mat_d, sizeof(float) * M_padded, A.mat_d,
			sizeof(float) * M, sizeof(float) * M, N, cudaMemcpyDeviceToDevice);
	if (err != cudaSuccess) {
...
void copy_matrix_to_device_padded(matrix A, matrix Apad) {
	...
	cudaError_t err;
	err = cudaMemcpy2D(Apad.mat_d, sizeof(float) * M_padded, A.mat,
			sizeof(float) * M, sizeof(float) * M, N, cudaMemcpyHostToDevice);
	if (err != cudaSuccess) {
		fprintf(stderr, "copy_to_padded: error in cudaMemcpy2D [%i],%i\n", err,
				cudaErrorInvalidValue);
		exit(1);
...
void copy_from_padded(matrix A, matrix Apad) {
	...
	if (M > M_padded) {
		fprintf(stderr,
				"copy_from_padded: padded number of rows must be >= original\n");
		exit(1);
	}
	if (N > N_padded) {
		fprintf(stderr,
				"copy_from_padded: padded number of cols must be >= original\n");
		exit(1);
	}

	cudaMemcpy2D(A.mat_d, sizeof(float) * M, Apad.mat_d,
			sizeof(float) * M_padded, sizeof(float) * M, N,
			cudaMemcpyDeviceToDevice);
...
void copy_matrix_from_device_padded(matrix A, matrix Apad) {
	...
	cudaMemcpy2D(A.mat, sizeof(float) * M, Apad.mat_d, sizeof(float) * M_padded,
...
void allocate_matrix_on_device(matrix* A) {
	...
	if (A->mat == NULL) {
		fprintf(stderr,
				"allocate_matrix_on_device: matrix not allocated on host\n");
		exit(1);
	}
	if (A->mat_d == NULL) {
		err = cudaMalloc((void**) &(A->mat_d), sizeof(float) * N);
		...
		fprintf(stderr,
				"allocate_matrix_on_device: matrix already allocated on device");
		exit(1);
	}
...
float matrix_difference_norm_d(action_t action, matrix a, matrix b,
		int* params) {
	...
	dim3 dimGrid2(
			(dimGrid.x / (block2 * lapt2))
					+ (!(dimGrid.x % (block2 * lapt2)) ? 0 : 1), 2);
	...
	if (block2 <= 1) { //if we only need one level of reduction
		...
		switch (block1) {
		case 1024:
			reduce1DDiff<1024> <<<dimGrid, dimBlock,
					2 * dimBlock.x * sizeof(float)>>>(a.mat_d, b.mat_d,
					result_d, N);
			break;
		case 512:
			reduce1DDiff<512> <<<dimGrid, dimBlock,
					2 * dimBlock.x * sizeof(float)>>>(a.mat_d, b.mat_d,
					result_d, N);
			break;
		case 256:
			reduce1DDiff<256> <<<dimGrid, dimBlock,
					2 * dimBlock.x * sizeof(float)>>>(a.mat_d, b.mat_d,
					result_d, N);
			break;
		case 128:
			reduce1DDiff<128> <<<dimGrid, dimBlock,
					2 * dimBlock.x * sizeof(float)>>>(a.mat_d, b.mat_d,
					result_d, N);
			break;
		case 64:
			reduce1DDiff<64> <<<dimGrid, dimBlock,
					2 * dimBlock.x * sizeof(float)>>>(a.mat_d, b.mat_d,
					result_d, N);
			break;
		case 32:
			reduce1DDiff<32> <<<dimGrid, dimBlock,
					2 * dimBlock.x * sizeof(float)>>>(a.mat_d, b.mat_d,
					result_d, N);
			break;
		case 16:
			reduce1DDiff<16> <<<dimGrid, dimBlock,
					2 * dimBlock.x * sizeof(float)>>>(a.mat_d, b.mat_d,
					result_d, N);
			break;
		case 8:
			reduce1DDiff<8> <<<dimGrid, dimBlock, 2 * dimBlock.x * sizeof(float)>>>(
					a.mat_d, b.mat_d, result_d, N);
			break;
		}
		...
		case 1024:
			reduce1DDiff<1024> <<<dimGrid, dimBlock,
					2 * dimBlock.x * sizeof(float)>>>(a.mat_d, b.mat_d, r1, N);
			break;
		case 512:
			reduce1DDiff<512> <<<dimGrid, dimBlock,
					2 * dimBlock.x * sizeof(float)>>>(a.mat_d, b.mat_d, r1, N);
		...
		case 8:
			reduce1DDiff<8> <<<dimGrid, dimBlock, 2 * dimBlock.x * sizeof(float)>>>(
					a.mat_d, b.mat_d, r1, N);
			break;
		}
		switch (block2) {
		case 1024:
			reduce2D<1024> <<<dimGrid2, dimBlock2, dimBlock2.x * sizeof(float)>>>(
					r1, result_d, dimGrid.x);
			break;
		case 512:
			reduce2D<512> <<<dimGrid2, dimBlock2, dimBlock2.x * sizeof(float)>>>(
					r1, result_d, dimGrid.x);
			break;
		case 256:
			reduce2D<256> <<<dimGrid2, dimBlock2, dimBlock2.x * sizeof(float)>>>(
					r1, result_d, dimGrid.x);
			break;
		case 128:
			reduce2D<128> <<<dimGrid2, dimBlock2, dimBlock2.x * sizeof(float)>>>(
					r1, result_d, dimGrid.x);
			break;
		case 64:
			reduce2D<64> <<<dimGrid2, dimBlock2, dimBlock2.x * sizeof(float)>>>(
					r1, result_d, dimGrid.x);
			break;
		case 32:
			reduce2D<32> <<<dimGrid2, dimBlock2, dimBlock2.x * sizeof(float)>>>(
					r1, result_d, dimGrid.x);
			break;
		case 16:
			reduce2D<16> <<<dimGrid2, dimBlock2, dimBlock2.x * sizeof(float)>>>(
					r1, result_d, dimGrid.x);
			break;
		case 8:
			reduce2D<8> <<<dimGrid2, dimBlock2, dimBlock2.x * sizeof(float)>>>(
					r1, result_d, dimGrid.x);
			break;
		}
	}
...
float matrix_div_d(action_t action, matrix a, matrix b, int* params) {
	...
	if (block2 <= 1) { //if we only need one level of reduction
		...
		switch (block1) {
		case 1024:
			reduce1DDiv<1024> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, b.mat_d, result_d, N);
			break;
		case 512:
			reduce1DDiv<512> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, b.mat_d, result_d, N);
			break;
		case 256:
			reduce1DDiv<256> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, b.mat_d, result_d, N);
			break;
		case 128:
			reduce1DDiv<128> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, b.mat_d, result_d, N);
			break;
		case 64:
			reduce1DDiv<64> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, b.mat_d, result_d, N);
			break;
		case 32:
			reduce1DDiv<32> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, b.mat_d, result_d, N);
			break;
		case 16:
			reduce1DDiv<16> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, b.mat_d, result_d, N);
			break;
		case 8:
			reduce1DDiv<8> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, b.mat_d, result_d, N);
			break;
		}
	} else { //if we need two levels of reduction
...
float nan_check_d(action_t action, matrix a, int* params) {
	...
	if (block2 <= 1) { //if we only need one level of reduction
		...
		switch (block1) {
		case 1024:
			reduce1DNan<1024> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, result_d, N);
			break;
		case 512:
			reduce1DNan<512> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, result_d, N);
			break;
		case 256:
			reduce1DNan<256> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, result_d, N);
			break;
		case 128:
			reduce1DNan<128> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, result_d, N);
			break;
		case 64:
			reduce1DNan<64> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, result_d, N);
			break;
		case 32:
			reduce1DNan<32> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, result_d, N);
			break;
		case 16:
			reduce1DNan<16> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, result_d, N);
			break;
		case 8:
			reduce1DNan<8> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, result_d, N);
			break;
		}
	} else { //if we need two levels of reduction
...
float zero_check_d(action_t action, matrix a, int* params) {
	...
	if (block2 <= 1) { //if we only need one level of reduction
		...
		switch (block1) {
		case 1024:
			reduce1DEql<1024> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, result_d, N);
			break;
		case 512:
			reduce1DEql<512> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, result_d, N);
			break;
		case 256:
			reduce1DEql<256> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, result_d, N);
			break;
		case 128:
			reduce1DEql<128> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, result_d, N);
			break;
		case 64:
			reduce1DEql<64> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, result_d, N);
			break;
		case 32:
			reduce1DEql<32> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, result_d, N);
			break;
		case 16:
			reduce1DEql<16> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, result_d, N);
			break;
		case 8:
			reduce1DEql<8> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, result_d, N);
			break;
		}
	} else { //if we need two levels of reduction
...
void matrix_multiply_d(matrix a, matrix b, matrix c) {
	cublasSgemm('N', 'N', c.dim[0], c.dim[1], a.dim[1], 1, a.mat_d, a.dim[0],
			b.mat_d, b.dim[0], 0, c.mat_d, c.dim[0]);
...
void matrix_multiply_AtB_d(matrix a, matrix b, matrix c) {
	cublasSgemm('T', 'N', c.dim[0], c.dim[1], b.dim[0], 1, a.mat_d, a.dim[0],
			b.mat_d, b.dim[0], 0, c.mat_d, c.dim[0]);
	if (cublasGetError() != CUBLAS_STATUS_SUCCESS) {
...
void matrix_multiply_ABt_d(matrix a, matrix b, matrix c) {
	cublasSgemm('N', 'T', c.dim[0], c.dim[1], a.dim[1], 1, a.mat_d, a.dim[0],
			b.mat_d, b.dim[0], 0, c.mat_d, c.dim[0]);
...
void row_divide_d(matrix a, matrix b, matrix c) {
	...
	if (a.dim[1] != b.dim[1] || a.dim[0] != c.dim[0] || a.dim[1] != c.dim[1]
			|| b.dim[0] != 1) {
...
void col_divide_d(matrix a, matrix b, matrix c) {
	...
	if (a.dim[0] != b.dim[0] || a.dim[0] != c.dim[0] || a.dim[1] != c.dim[1]
			|| b.dim[1] != 1) {
...
void sum_cols_d(action_t action, matrix a, matrix c, int* params) {
	...
	if (block2 <= 1) { //if we only need one level of reduction
		...
		switch (block1) {
		case 1024:
			reduce2D<1024> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, c.mat_d, N);
			break;
		case 512:
			reduce2D<512> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, c.mat_d, N);
			break;
		case 256:
			reduce2D<256> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, c.mat_d, N);
			break;
		case 128:
			reduce2D<128> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, c.mat_d, N);
			break;
		case 64:
			reduce2D<64> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, c.mat_d, N);
			break;
		case 32:
			reduce2D<32> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, c.mat_d, N);
			break;
		case 16:
			reduce2D<16> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, c.mat_d, N);
			break;
		case 8:
			reduce2D<8> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, c.mat_d, N);
			break;
		}
	} else { //if we need two levels of reduction
...
void sum_rows_d(action_t action, matrix a, matrix c, int* params) {
	...
	if (block2 <= 1) { //if we only need one level of reduction
		...
		switch (block1) {
		case 1024:
			reduce2DStrided<1024> <<<dimGrid, dimBlock,
					dimBlock.x * sizeof(float)>>>(a.mat_d, c.mat_d, N, M);
			break;
		case 512:
			reduce2DStrided<512> <<<dimGrid, dimBlock,
					dimBlock.x * sizeof(float)>>>(a.mat_d, c.mat_d, N, M);
			break;
		case 256:
			reduce2DStrided<256> <<<dimGrid, dimBlock,
					dimBlock.x * sizeof(float)>>>(a.mat_d, c.mat_d, N, M);
			break;
		case 128:
			reduce2DStrided<128> <<<dimGrid, dimBlock,
					dimBlock.x * sizeof(float)>>>(a.mat_d, c.mat_d, N, M);
			break;
		case 64:
			reduce2DStrided<64> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, c.mat_d, N, M);
			break;
		case 32:
			reduce2DStrided<32> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, c.mat_d, N, M);
			break;
		case 16:
			reduce2DStrided<16> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, c.mat_d, N, M);
			break;
		case 8:
			reduce2DStrided<8> <<<dimGrid, dimBlock, dimBlock.x * sizeof(float)>>>(
					a.mat_d, c.mat_d, N, M);
			break;
		}
	} else { //if we need two levels of reduction
		...
		case 1024:
			reduce2DStrided<1024> <<<dimGrid, dimBlock,
					dimBlock.x * sizeof(float)>>>(a.mat_d, r1, N, M);
			break;
		case 512:
			reduce2DStrided<512> <<<dimGrid, dimBlock,
					dimBlock.x * sizeof(float)>>>(a.mat_d, r1, N, M);
		...
		switch (block2) {
		case 1024:
			reduce2DStrided<1024> <<<dimGrid2, dimBlock2,
					dimBlock2.x * sizeof(float)>>>(r1, c.mat_d, dimGrid.x, M);
			break;
		case 512:
			reduce2DStrided<512> <<<dimGrid2, dimBlock2,
					dimBlock2.x * sizeof(float)>>>(r1, c.mat_d, dimGrid.x, M);
			break;
		case 256:
			reduce2DStrided<256> <<<dimGrid2, dimBlock2,
					dimBlock2.x * sizeof(float)>>>(r1, c.mat_d, dimGrid.x, M);
			break;
		case 128:
			reduce2DStrided<128> <<<dimGrid2, dimBlock2,
					dimBlock2.x * sizeof(float)>>>(r1, c.mat_d, dimGrid.x, M);
			break;
		case 64:
			reduce2DStrided<64> <<<dimGrid2, dimBlock2,
					dimBlock2.x * sizeof(float)>>>(r1, c.mat_d, dimGrid.x, M);
			break;
		case 32:
			reduce2DStrided<32> <<<dimGrid2, dimBlock2,
					dimBlock2.x * sizeof(float)>>>(r1, c.mat_d, dimGrid.x, M);
			break;
		case 16:
			reduce2DStrided<16> <<<dimGrid2, dimBlock2,
					dimBlock2.x * sizeof(float)>>>(r1, c.mat_d, dimGrid.x, M);
			break;
		case 8:
			reduce2DStrided<8> <<<dimGrid2, dimBlock2,
					dimBlock2.x * sizeof(float)>>>(r1, c.mat_d, dimGrid.x, M);
			break;
		}
	}
...
__global__ void reduce2DStrided(float *g_idata, float *g_odata, int N,
	...
	while (i < n) {
		sdata[tid] += g_idata[i * stride + offset]
				+ g_idata[(i + blockSize) * stride + offset];
		i += gridSize;
	}
	if (i < N)
		sdata[tid] += g_idata[i * stride + offset];
	...
	// write result for this block to global mem
	if (tid == 0)
		g_odata[blockIdx.y + blockIdx.x * gridDim.y] = sdata[0];
}
...
int most_interesting_column(matrix a) {
	...
	vector column_maxs;
	create_vector(&column_maxs, cols, 0);
	...
	for (int i = 0; i < cols; i++) {
		if (column_maxs.vec[i] > max_val) {
			max_val = column_maxs.vec[i];
			max_index = i;
		}
	}
